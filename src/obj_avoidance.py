...
class runRobot():  # main class
    ...
    def lidar_callback(self, msg):  # function for obstacle avoidance
        # "Safe distance"
        self.distance = 1.3
        # Only act based on lidar data if it's not stuck
        if self.is_stuck == False:
            ...
            if msg.ranges[0] > self.distance and msg.ranges[15] > self.distance and msg.ranges[345] > self.distance:
                ...
                # Check Obstacle on the right
                    ...
                    self.twist.angular.z = 0.50  # Turning left or right, doesn't matter
    ...
    def odom_callback(self, msg):
        ...
        COUNTER_LIMIT = 20
        if self.last_pos[0] == round(msg.pose.pose.position.x, 2) and self.last_pos[1] == round(msg.pose.pose.position.y, 2):
            ...
            self.last_pos[0] = round(msg.pose.pose.position.x, 2)
            self.last_pos[1] = round(msg.pose.pose.position.y, 2)
            self.last_pos[2] = 0
            self.is_stuck = False
